...
int main(void)
{
	...
	OLED_ShowString(0, 0, "MPU6050_Yaw:", OLED_8X16);
	OLED_ShowString(0, 32, "HWT101_Yaw:", OLED_8X16);
	OLED_Update();
	while(1)
	{
		MPU6050_DMP_Get_Data(&Pitch,&Roll,&Yaw);
		OLED_Printf(32, 16, OLED_8X16, "%+04.lf", Yaw);
		if (Serial_GetRxFlag() == 1)	//如果接收到数据包
		{
			OLED_ShowFloatNum(32, 48, (double)read_Yaw(Serial_RxPacket[4], Serial_RxPacket[5]), 3, 3, OLED_8X16);
			OLED_Update();
		}	
		OLED_Update();
	}
