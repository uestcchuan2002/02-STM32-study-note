...
TIM_HandleTypeDef tim7_handler;         /* 定时器参数句柄 */

uint32_t FreeRTOSRunTimeTicks;

void ConfigureTimeForRunTimeStats(void)
{
	btim_timx_int_init(10-1, 84-1);
...
{
    HAL_TIM_IRQHandler(&tim7_handler);  /*定时器回调函数*/
}
...
{
    tim7_handler.Instance = BTIM_TIM7_INT;                      /*通用定时器X*/
    tim7_handler.Init.Prescaler = psc;                          /* 设置预分频器  */
    ...
    HAL_TIM_Base_Start_IT(&tim7_handler);                       /*使能通用定时器x和及其更新中断：TIM_IT_UPDATE*/
}
